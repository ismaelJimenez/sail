...
int main(int argc, const char **argv)
{
  try {
    ...
    // Note: CLI11 will automatically handle remaining arguments after -- for us
    
    // Helper function to get executable path with correct extension
    auto get_executable_path = [](const std::filesystem::path& target_dir, const std::string& project_name) -> std::filesystem::path {
      return target_dir / (project_name + EXECUTABLE_EXTENSION);
    };
    
    // Helper function to quote paths for system commands
    auto quote_path = [](const std::string& path) -> std::string {
#ifdef _WIN32
      return "\"" + path + "\"";
#else
    ...
    auto build_project = [&](bool release_mode) -> std::pair<int, std::filesystem::path> {
      // Find project root by looking for Sail.toml
      std::filesystem::path project_root;
      std::filesystem::path current_path = std::filesystem::current_path();
      
      while (current_path != current_path.root_path()) {
        std::filesystem::path sail_toml_path = current_path / "Sail.toml";
      ...
      std::filesystem::path sail_toml_path = project_root / "Sail.toml";
      ...
      std::string build_mode = release_mode ? "Release" : "Debug";
      std::string target_subdir = release_mode ? "release" : "debug";
      std::filesystem::path target_dir = project_root / "target" / target_subdir;
      ...
      try {
        ...
        std::filesystem::path cmake_path = project_root / "CMakeLists.txt";
...
set_target_properties({} PROPERTIES OUTPUT_NAME "{}")
        ...
        std::filesystem::path build_dir = target_dir / "build";
        std::filesystem::create_directories(build_dir);
        
        // Run CMake configure
        std::string cmake_configure_cmd = fmt::format(
        ...
        int configure_result = std::system(cmake_configure_cmd.c_str());
        ...
        // Run CMake build
        std::string cmake_build_cmd = fmt::format(
          "cmake --build {} --config {}",
          quote_path(build_dir.string()),
          build_mode
        );
        ...
        std::filesystem::path executable_path = get_executable_path(target_dir, project_name);
    ...
    auto run_handler = [&]() {
      ...
      auto [build_result, executable_path] = build_project(run_release);
    ...
    run_subcommand->callback(run_handler);
    
    auto build_handler = [&]() {
      fmt::print("Configuring project...\n");
      fmt::print("Compiling...\n");
      
      auto [build_result, executable_path] = build_project(build_release);
      if (build_result != EXIT_SUCCESS) {
        return build_result;
      }
      
      std::string target_subdir = build_release ? "release" : "debug";
      std::string build_mode = build_release ? "Release" : "Debug";
    ...
    build_subcommand->callback(build_handler);
    
    auto new_handler = [&]() {
      std::filesystem::path project_dir = std::filesystem::current_path() / new_project_name;
      ...
      try {
        ...
        // Create Sail.toml
        std::filesystem::path sail_toml_path = project_dir / "Sail.toml";
...
int main() {
    ...
    new_subcommand->callback(new_handler);
...
[dependencies]
      ...
      std::filesystem::path sail_toml_path = std::filesystem::current_path() / "Sail.toml";
    ...
    init_subcommand->callback(init_handler);
